...
import { symbolUnitMultiplier } from "./position-utils.js";
...
async function fetchBinance(env, startMs, endMs) {
  ...
  for (const row of trades) {
    ...
    const quantity = Number(row.qty || 0) * multiplier;
    const normalizedPrice = Number(row.price || 0) / multiplier;
    ...
    group.fillNotional += quantity * normalizedPrice;
...
export function normalizePair(symbol) {
  ...
  if (pair === "1000LUNC" || pair === "LUNC") return "LUNC";
  if (pair === "INTCUS") return "INTC";
  if (pair === "AMZNUS") return "AMZN";
