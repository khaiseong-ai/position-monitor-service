...
export function unexpectedEmptyPositionSources(positions, configuredSources = "") {
  const required = String(configuredSources || "")
    .split(",")
    .map((value) => value.trim().toLowerCase())
    .filter(Boolean);
  const present = new Set((positions || []).map((position) => String(position.source || "").toLowerCase()));
  return [...new Set(required)].filter((source) => !present.has(source));
}
...
async function fetchSheetEndpoint(parsedUrl, options, {
  fetchImpl,
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      if (response.ok || !isRetryableSheetStatus(response.status) || attempt === maxAttempts) {
        return response;
      }
    } catch (error) {
      lastError = error;
...
export async function writePositionSheet({
  url,
  secret,
  snapshot,
  fetchImpl = fetch,
  ...
  const response = await fetchSheetEndpoint(parsedUrl, {
    method: "POST",
  ...
  }, { fetchImpl, sleepImpl, maxAttempts });
...
export async function readPositionSheetIgnores({
  url,
  secret,
  fetchImpl = fetch,
  sleepImpl,
