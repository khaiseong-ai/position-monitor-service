...
import { makeOrder, makePosition, numberValue, normalizeSymbol, symbolUnitMultiplier } from "./position-utils.js";
...
export async function fetchBinance(config) {
  ...
  return json.map((row) => {
    ...
    const multiplier = symbolUnitMultiplier(row.symbol);
    return makePosition({
      symbol: row.symbol,
      source: "binance",
      side: amount > 0 ? "long" : "short",
      size: amount * multiplier,
      price: numberValue(row.markPrice) / multiplier,
      raw: row
    });
  }).filter(Boolean);
}
...
export async function fetchBinanceOrders(config) {
  ...
  return [...openOrders, ...asArray(algoOrders)].map((row) => {
    const multiplier = symbolUnitMultiplier(row.symbol);
    return makeOrder({
      symbol: row.symbol,
      source: "binance",
      side: row.side,
      size: numberValue(row.origQty || row.quantity || row.qty) * multiplier,
      price: numberValue(row.price) / multiplier,
      triggerPrice: numberValue(row.stopPrice || row.triggerPrice) / multiplier,
...
async function mexcGet(config, path, params = {}) {
  ...
  const json = await readJson(response, `mexc ${path}`);
  if (json.success === false || (json.code !== undefined && Number(json.code) !== 0)) {
    throw new Error(`mexc ${path} error: ${json.code ?? "unknown"}`);
  }
  return json;
}
...
export async function fetchPhemex(config) {
  ...
  if (!gate.ok) return [];
  const primary = await phemexGet(config, "/g-accounts/accountPositions", { currency: config.currency });
  const primaryPositions = mapPhemexPositions(primary);
  if (primaryPositions.length > 0) return primaryPositions;

  const realtime = await phemexGet(config, "/g-accounts/positions", { currency: config.currency });
  return mapPhemexPositions(realtime);
}

function mapPhemexPositions(json) {
