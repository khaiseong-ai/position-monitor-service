...
export async function fetchPositionRelay(config, fetchImpl = fetch) {
  ...
  const token = await relayAuthorizationToken(config.token, fetchImpl);
  if (!token) throw new Error("position relay authorization is missing");
...
async function relayAuthorizationToken(fallbackToken, fetchImpl) {
  ...
  if (requestUrl && requestToken) {
    ...
    if (!response.ok || !String(body?.value || "").trim()) {
      throw new Error("position relay OIDC authorization failed");
    }
